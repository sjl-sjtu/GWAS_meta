...
simula <- function(trueOR,n_studies,p,f,n_samples_lower,n_samples_upper,
                   ...
                   mcmcIter=10000,shotgunIter=500,B=5,repeats=10){
  library(meta)
...
n_studies <- 20
trueOR <- Reduce(union,c(seq(0.5,2.0,0.1),seq(0.86,1.1,0.02),seq(0.94,1.05,0.01)))
trueOR <- sort(trueOR)
dfAll <- simula(trueOR,n_studies,0.05,0.8,100,5000,0.4,"correlated",0.5,mcmcIter=10000,shotgunIter=500,B=5,repeats=10)
write.csv(dfAll,file = "sim1.csv",na = "NA")

n_studies <- 25
trueOR <- Reduce(union,c(seq(0.5,2.0,0.1),seq(0.86,1.1,0.02),seq(0.94,1.05,0.01)))
trueOR <- sort(trueOR)
dfAll1 <- simula(trueOR,n_studies,0.05,0.8,100,5000,0.4,"correlated",0.5,mcmcIter=10000,shotgunIter=500,B=5,repeats=10)
write.csv(dfAll1,file = "sim1-1.csv",na = "NA")
